...
const STATUS_COLORS: Record<string, string> = {
    draft: "#94a3b8",
    ready: "#10b981",
    "battle-tested": "#f59e0b",
};

export default function ContextConfirmationPage() {
    const { t } = useLanguage();
    const statusLabel = (s: string) => ({
        draft: t('confirm.status.draft'),
        ready: t('confirm.status.ready'),
        'battle-tested': t('confirm.status.battleTested'),
    }[s] ?? t('confirm.status.draft'));
            ...
            <main className="flex-1 max-w-[900px] w-full mx-auto p-6 lg:py-12 flex flex-col">
                ...
                <div className="text-center mb-8">
                    <div className="flex items-center justify-center gap-3 mb-3">
                        ...
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">{t('confirm.title')}</h1>
                    </div>
                    <p className="text-slate-500 text-sm">{t('confirm.subtitle')}</p>
                </div>
                ...
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${showAll ? 'bg-teal-500 text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:border-teal-300'}`}
                    >
                        {t('confirm.all')} ({allStories.length})
                    </button>
                    {showAll && (
                ...
                <div className="flex-1 mb-8">
                    {storiesLoading ? (
                        <div className="flex items-center justify-center py-16">
                            ...
                            <span className="text-sm text-slate-400">{t('confirm.loading')}</span>
                        </div>
                    ) : filteredStories.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            <BookOpen className="w-12 h-12 text-slate-200 mb-4" />
                            <p className="text-slate-500 font-medium mb-1">
                                {hasStories ? t('confirm.noMatch') : t('confirm.noStories')}
                            </p>
                            <p className="text-slate-400 text-sm mb-4">
                                {hasStories ? t('confirm.noMatch.sub') : t('confirm.noStories.sub')}
                            </p>
                            <button
                                onClick={() => router.push('/stories/create?returnTo=/setup/confirm')}
                        ...
                        <div className="space-y-3">
                            {filteredStories.map((story: any) => {
                                ...
                                const statusColor = STATUS_COLORS[story.status] || STATUS_COLORS.draft;
                                const statusText = statusLabel(story.status);
                                        ...
                                        <div className="flex items-center gap-4 px-5 py-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: `${statusColor}20`, color: statusColor }}>
                ...
                <div className="sticky bottom-0 bg-[#f8fafc] pt-4 pb-6 border-t border-slate-200 flex items-center justify-between gap-4">
                    <div className="text-sm text-slate-500">
                        {selectedStoryIds.length > 0
                            ? <span className="font-medium text-teal-600">{selectedStoryIds.length} {t('confirm.selected')}</span>
                            : t('confirm.noneSelected')
                        }
                    </div>
