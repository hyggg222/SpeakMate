...
import { LogOut, KeyRound, Trash2, CheckCircle2, Loader2, User as UserIcon } from 'lucide-react'
                ...
                <div className="max-w-2xl mx-auto space-y-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <h2 className="text-base font-bold text-slate-800 mb-5 flex items-center gap-2">
                            <UserIcon className="w-4 h-4 text-teal-500" /> Hồ sơ
                        </h2>
                        ...
                        <div className="flex items-center gap-4 mb-6 p-4 bg-slate-50 rounded-xl">
                            ...
                            <div>
                                <p className="font-semibold text-slate-800 text-sm">
                                    {user?.user_metadata?.full_name || displayName || 'Người dùng'}
                                </p>
                                <p className="text-xs text-slate-500 mt-0.5">{user?.email || 'Chưa đăng nhập'}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    {nameSaved ? <><CheckCircle2 className="w-4 h-4" /> Đã lưu</> : 'Lưu'}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <h2 className="text-base font-bold text-slate-800 mb-5 flex items-center gap-2">
                            <KeyRound className="w-4 h-4 text-teal-500" /> Tài khoản & Bảo mật
                        </h2>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="p-4 bg-slate-50 rounded-xl">
                                ...
                                <p className="text-xs text-slate-500 mb-3">
                                    Gửi email đặt lại mật khẩu tới <span className="font-medium">{user?.email}</span>
                                </p>
                                {resetSent ? (
                                    <div className="flex items-center gap-2 text-teal-600 text-sm font-medium">
                                        <CheckCircle2 className="w-4 h-4" /> Email đã gửi — kiểm tra hộp thư của bạn
                                    </div>
                                ) : (
                                    <>
                                        <button
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <h2 className="text-base font-bold text-slate-800 mb-5 flex items-center gap-2">
                            <Trash2 className="w-4 h-4 text-slate-400" /> Dữ liệu & Tài khoản
                        </h2>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                <div>
                                    <p className="text-sm font-semibold text-slate-700">Xóa dữ liệu demo</p>
                                    <p className="text-xs text-slate-500 mt-0.5">Xóa dữ liệu mẫu được tạo tự động khi vào app lần đầu</p>
                                </div>
                                <button
                                ...
                                >
                                    {demoCleared
                                        ? <><CheckCircle2 className="w-3.5 h-3.5 text-teal-500" /> Đã xóa</>
